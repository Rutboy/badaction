...
Представление группы вычисляется без дублирования сохранённых данных карточек.
Если название группы не задано, используется текст основной карточки. Общий
счётчик голосов группы вычисляется через `COUNT(DISTINCT visitor_token)` по
голосам всех карточек группы. При создании задачи из группы копируется это
отображаемое название, а для существующей необязательной связи с исходной
карточкой используется основная карточка.
