...
import { serializeTargetJsonExport } from "@/lib/export/target-export";
...
export async function GET(request: Request, { params }: ExportRouteContext) {
  return handleContentRoute(async () => {
    ...
    return new Response(serializeTargetJsonExport(data), {
      headers: {
        "Cache-Control": "no-store",
        "Content-Type": "application/json; charset=utf-8",
        "Content-Disposition": `attachment; filename="retro-${boardId}.json"`,
      },
    });
  });
}
